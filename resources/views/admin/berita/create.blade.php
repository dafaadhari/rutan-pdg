...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl font-bold text-white tracking-tight">Tulis Berita Baru</h1>
            <p class="text-sm text-gray-400 mt-1">Publikasikan rilis atau pengumuman giat resmi Rutan Pandeglang.</p>
        </div>
    </div>
            ...
            <div>
                ...
                <div class="mt-2">
                    <input type="text" name="judul" id="judul" required
                           value="{{ old('judul') }}"
                           placeholder="Contoh: Rutan Pandeglang Gelar Pelatihan Kemandirian WBP Bidang Pertanian"
                ...
                <div class="mt-2 text-xs text-gray-500 flex items-center gap-1 font-mono">
                    <span>URL Slug:</span>
                    <span id="slug-preview" class="text-amber-400/90 font-medium">/berita/...</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <select name="status" id="status" required
                            ...
                            <option value="publikasi" {{ old('status', 'publikasi') === 'publikasi' ? 'selected' : '' }}>
                                Langsung Terbitkan (Publikasi)
                            </option>
                    ...
                    @error('status')
                        <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                    @enderror
                    <p class="mt-1.5 text-xs text-gray-500">Berita dengan status publikasi akan langsung tampil di halaman depan portal.</p>
                </div>
                ...
                <div>
                    ...
                    @error('gambar_sampul')
                        <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                    @enderror
                    <p class="mt-1.5 text-xs text-gray-500">Format: JPG, PNG, WEBP. Ukuran maksimal 2 MB.</p>
                </div>
            </div>

            <!-- Preview Box Image -->
            <div id="image-preview-container" class="hidden">
                <label class="block text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Pratinjau Sampul</label>
                <div class="relative w-full max-w-sm h-48 rounded-xl overflow-hidden border border-gray-700 bg-gray-850">
            ...
            <div>
                ...
                <div class="mt-2">
                    <textarea name="konten" id="konten" rows="12" required
                              placeholder="Tuliskan isi rilis berita lengkap di sini..."
                              class="w-full px-4 py-3 bg-gray-850 border {{ $errors->has('konten') ? 'border-red-500' : 'border-gray-700' }} rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 text-sm leading-relaxed transition">{{ old('konten') }}</textarea>
                </div>
                @error('konten')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Tips: Gunakan enter untuk memisahkan antar paragraf agar mudah dibaca pengunjung.</p>
            </div>
...
<script>
    // Live Auto-Slug Generator Preview
